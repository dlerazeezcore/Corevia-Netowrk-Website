...
export function MyEsims() {
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-full bg-gradient-to-b from-gray-50 via-blue-50/30 to-purple-50/30 pb-6 overflow-auto"
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 flex-1">
                      <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center shadow-sm">
                        <CountryFlag
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 mb-1">{esim.name}</h3>
                  ...
                  {esim.dataTotal > 0 && (
                    <div className="mb-4 p-4 rounded-xl bg-gradient-to-br from-gray-50 to-blue-50/50">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <Database className="w-4 h-4 text-primary" />
                          <span className="text-xs font-medium text-gray-700">Data Usage</span>
                        </div>
                  ...
                  {esim.dataTotal === 0 && esim.status !== "expired" && (
                    <div className="mb-4 p-4 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-gray-900">Unlimited Data</p>
                          <p className="text-xs text-gray-600">Use as much as you need</p>
                        </div>
                      </div>
                    </div>
                  )}

                  <div className="flex items-center justify-between p-3 rounded-lg bg-white border border-gray-100 mb-4">
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-gray-400" />
                      <span className="text-xs text-gray-600">Valid until</span>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900">{formatDate(esim.validUntil)}</p>
                      {!esim.activatedDate ? (
                        <p className="text-xs text-gray-500">Not activated yet</p>
                      ) : esim.status === "expired" || (esim.hasDaysLeft && esim.daysLeft <= 0) ? (
                        <p className="text-xs text-red-600">Expired/Inactive</p>
                      ) : esim.hasDaysLeft && esim.daysLeft > 0 ? (
                        <p className="text-xs text-gray-500">{esim.daysLeft} days left</p>
                      ) : (
